...
void misc::VisualGraph::BFTraversal(int startNode)
{
	traversal.clear();

	std::queue<int> frontier;
	frontier.push(startNode);
	traversal[startNode] = -1;

	while (!frontier.empty())
	{
		std::queue<int> newFrontier;
		...
		while (!frontier.empty())
		{
			...
			auto it = mInternalGraph.find(currentNode);
			if (it != mInternalGraph.end())
			{
				const auto& currentNeighborsMap = (*it).second;
				for (const auto& neighborPair : currentNeighborsMap)
				{
					int neighbor = neighborPair.first;
					if (traversal.find(neighbor) == traversal.end())
					{
						traversal[neighbor] = currentNode;
...
void misc::VisualGraph::DFTraversal(int startNode)
{
	...
	while (!frontier.empty())
	{
		...
		if (traversal.find(currentNode) != traversal.end()) 
			continue;

		traversal[currentNode] = -1; 
